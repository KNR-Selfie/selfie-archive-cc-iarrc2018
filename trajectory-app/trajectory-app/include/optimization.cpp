    ...
    for(int j=0;j<number_of_rec_raws;j++)//przejscie do kolejnego pasa //jeden do gory
    {
        ...
        if(find_flag == 0)//szukaj po rzedach poki nie znajdzie punktow
        {
            for(int i=0;i<number_of_rec_cols;i++)//przejscie po jednym pasie //tylko za pierwszym razem!
            {
                ...
                for(int k=LIDAR_MAT_HEIGHT-(j+1)*rec_height;k<LIDAR_MAT_HEIGHT-j*rec_height;k++)
                {
            ...
            if(max_pixel_count!=0)
            {
                ...
                Rect search_rectangle(max_pixel_rec_index*rec_width,LIDAR_MAT_HEIGHT-(j+1)*rec_height,rec_width,rec_height);
        ...
        {       //petla do wykrycia rozkładu pixeli
                ...
                {//przeszukiwanie pasa tylko w zakresie [-param +param]
                    ...
                    for(int k=LIDAR_MAT_HEIGHT-(j+1)*rec_height;k<LIDAR_MAT_HEIGHT-j*rec_height;k++) //wysokość pasa
                    {
...
void line_search(int previous_angle, spline_t &path_line)
{
    ...
    if(previous_angle>0)
    {
        ...
        pom.push_back(Point(LIDAR_MAT_WIDTH/2,LIDAR_MAT_HEIGHT));
        pom.push_back(Point (LIDAR_MAT_WIDTH,LIDAR_MAT_HEIGHT/2));
        pom.push_back(Point(LIDAR_MAT_WIDTH,0));
    ...
    {
        pom.push_back(Point(LIDAR_MAT_WIDTH/2,LIDAR_MAT_HEIGHT));
        pom.push_back(Point (0,LIDAR_MAT_HEIGHT/2));
