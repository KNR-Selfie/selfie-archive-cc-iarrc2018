...
uint32_t gyroLooptime = 1000; // Gyro Loop Rate in Hz.

filter_t gyroLPF[3];
static uint8_t gyroLPF_Hz[3] = {90, 90, 80};

void gyro_write(uint8_t gyro_reg, uint8_t data);
void gyro_data_read(uint8_t reg_addr, uint8_t *data, uint32_t size);
...
void mpu6000AccAndGyroInit()
{
    ...
    gyro_write (MPU_RA_CONFIG, BITS_DLPF_CFG_256HZ);
    osDelay (1);
    ...
    gyro_write(MPU_RA_INT_PIN_CFG, 0 << 7 | 0 << 6 | 0 << 5 | 1 << 4 | 0 << 3 | 0 << 2 | 0 << 1 | 0 << 0);  // INT_ANYRD_2CLEAR
    osDelay(1);
...
void gyro_data_conv(uint8_t * data)  {
    gyro_raw[0] = data[2] << 8 | data[3];
    gyro_raw[1] = -(data[0] << 8 | data[1]);
